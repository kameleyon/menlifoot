...
const FILTERS = ['All', 'Apparel', 'Caps', 'Accessories'];
const PRODUCTS = [
    ...
    <AppShell>
        ...
        <div className="pt-14">
          <div className="flex items-end justify-between px-5 pb-[18px]">
            <div className="flex flex-col gap-2">
              <span className="font-sans text-[9px] font-semibold uppercase tracking-[0.18em] text-primary">Now open</span>
              <span className="font-display text-[30px] uppercase">Store</span>
            </div>
            <button onClick={() => setView('cart')} className="relative flex h-[38px] w-[38px] items-center justify-center rounded-full border border-white/[0.14]">
              <span className="font-sans text-[11px] font-medium text-foreground/80">Bag</span>
          ...
          <div className="flex gap-2 overflow-x-auto px-5 pb-[18px] [scrollbar-width:none]">
            {FILTERS.map((f, i) => (
              <button key={f} onClick={() => setActive(i)} className="flex-none rounded-full px-[13px] py-2 font-sans text-[11px] font-medium"
                style={active === i ? { background: '#f4f2ee', color: '#070708' } : { border: '1px solid rgba(255,255,255,.14)', color: 'rgba(244,242,238,.7)' }}>{f}</button>
            ))}
          </div>
            ...
            {PRODUCTS.map((p) => (
              <button key={p.name} onClick={() => openProduct(p)} className="flex flex-col gap-2 text-left">
                <div className="relative flex h-[176px] items-end justify-center rounded-xl pb-2.5" style={{ background: stripe }}>
                  ...
                  {p.limited && <span className="absolute left-2.5 top-2.5 rounded-full bg-primary px-[7px] py-1 font-sans text-[8px] font-bold uppercase tracking-[0.14em] text-[#070708]">Limited</span>}
                </div>
        ...
        <div className="pb-28">
          ...
          <div className="flex flex-col gap-[18px] px-5 pt-[22px]">
            ...
            <div className="flex flex-col gap-2.5">
              <span className="font-sans text-[9px] font-semibold uppercase tracking-[0.14em] text-foreground/45">Size</span>
          ...
          <div className="fixed inset-x-0 bottom-[76px] left-1/2 z-30 flex w-full max-w-[520px] -translate-x-1/2 items-center gap-3 px-5 pb-4 pt-3" style={{ background: 'linear-gradient(to top,#070708 55%,rgba(7,7,8,0))' }}>
            <button onClick={() => { setCart((c) => c + 1); setView('shop'); }} className="flex h-[52px] flex-1 items-center justify-center rounded-full font-sans text-[13.5px] font-bold tracking-[0.04em] text-[#070708]" style={{ background: 'linear-gradient(135deg,#e9c877,#c08a2a)' }}>Add · {selected.price}</button>
      ...
      {view === 'cart' && (
        <div className="pt-14">
          <div className="flex items-center gap-3 px-4 pb-5">
            ...
            <span className="font-display text-[22px] uppercase">Your bag</span>
          </div>
          {cart > 0 ? (
            <div>
              ...
              <div className="m-5 flex flex-col gap-2.5 rounded-2xl border border-white/[0.07] bg-[#101012] p-4">
                <div className="flex justify-between font-sans text-[12.5px] text-foreground/60"><span>Subtotal</span><span>${cart * 42}.00</span></div>
                <div className="flex justify-between font-sans text-[12.5px] text-foreground/60"><span>Shipping</span><span>$0.00</span></div>
                <div className="h-px bg-white/[0.07]" />
                <div className="flex items-baseline justify-between"><span className="font-sans text-[13px] font-semibold">Total</span><span className="font-display text-[20px] text-primary">${cart * 42}.00</span></div>
              </div>
              <button onClick={() => { setView('done'); setCart(0); }} className="mx-5 flex h-[52px] w-[calc(100%-40px)] items-center justify-center rounded-full font-sans text-[13.5px] font-bold tracking-[0.04em] text-[#070708]" style={{ background: 'linear-gradient(135deg,#e9c877,#c08a2a)' }}>Checkout · Apple Pay</button>
              <div className="mt-3.5 text-center font-sans text-[11px] text-foreground/35">Secure checkout · Stripe</div>
            </div>
          ) : (
            <div className="flex flex-col items-center gap-3.5 px-[30px] py-[60px] text-center">
              ...
              <span className="font-sans text-[15px] font-semibold">Nothing in the bag yet</span>
              <span className="font-sans text-[12.5px] leading-[1.6] text-foreground/50">The launch drop is live — caps, tees and the podcast crewneck.</span>
              <button onClick={() => setView('shop')} className="mt-1.5 rounded-full border border-primary/50 px-5 py-3 font-sans text-[12px] font-semibold text-primary">Browse the store</button>
            </div>
          )}
        </div>
      )}
      ...
      {view === 'done' && (
        <div className="flex min-h-[80dvh] flex-col items-center justify-center gap-4 px-[34px] text-center">
          ...
          <span className="font-display text-[26px] uppercase leading-[1.05]">Order confirmed</span>
          <span className="font-sans text-[13px] leading-[1.6] text-foreground/55">#MLF-2841 · confirmation sent. You'll get tracking within 24 hours.</span>
          <button onClick={() => setView('shop')} className="mt-2 rounded-full border border-white/[0.16] px-[22px] py-[13px] font-sans text-[12.5px] font-semibold">Back to store</button>
        </div>
      )}
    </AppShell>
  );
